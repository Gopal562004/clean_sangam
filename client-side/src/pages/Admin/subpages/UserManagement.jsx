...
//     try {
//       await approveOrRejectUser(userId, action);
//       setUsers(users.filter((u) => u._id !== userId)); // remove user from list after action
//     } catch (err) {
//       console.error(err);
//     }
//   };
...
//                   className="text-text-secondary mx-auto mb-3"
//                 />
//                 <p className="text-text-secondary">No pending users</p>
//               </td>
//             </tr>
//           ) : (
//             filteredUsers.map((user) => (
...
const UserManagement = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const roles = ["alumni", "student", "faculty", "recruiter"];
  ...
  const fetchPendingUsers = async () => {
    try {
      ...
      for (const role of roles) {
        const data = await getPendingUsers(role);
        roleData[role] = Array.isArray(data) ? data : data.users || [];
      }
      setPendingUsers(roleData);
    } catch (err) {
  ...
  const handleAction = async (userId, action) => {
    try {
      ...
      setPendingUsers((prev) => {
        const updated = { ...prev };
        for (const role of roles) {
          updated[role] = updated[role].filter((u) => u._id !== userId);
        }
        return updated;
      });
    } catch (err) {
  ...
  // Utility to determine table height
  const getTableHeight = (numRows, maxRows, rowHeight = 56) => {
    return numRows > maxRows ? maxRows * rowHeight : "auto";
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        style={{ maxHeight: getTableHeight(filteredUsers.length, 8) }}
      >
      ...
      {roles.map((role) => (
        <div key={role} className="mt-6">
          ...
          <div
            className="overflow-auto border border-border rounded-lg"
            style={{
              maxHeight: getTableHeight((pendingUsers[role] || []).length, 5),
            }}
          >
            <table className="w-full">
              <thead className="bg-muted">
              ...
              <tbody className="divide-y divide-border">
                {(pendingUsers[role] || []).length === 0 ? (
                  <tr>
                    <td colSpan="3" className="px-6 py-8 text-center">
                      <p className="text-text-secondary">No pending {role}</p>
                    </td>
                  </tr>
                ) : (
                  (pendingUsers[role] || []).map((user) => (
                    <tr key={user._id} className="hover:bg-muted/50">
                      <td className="px-6 py-4">{user.name}</td>
                      <td className="px-6 py-4">{user.email}</td>
                      <td className="px-6 py-4 space-x-2">
