...
const DonorRecognition = ({ donors }) => {
  ...
  // Automatically assign donor level based on amount
  const getDonorLevelConfig = (amount) => {
    if (amount >= 10000) {
      return {
        level: "Platinum",
    ...
    <div className="bg-card rounded-lg p-6 border border-border">
      ...
      <div className="space-y-4">
        {donors?.map((donor, index) => {
          ...
          const levelConfig = getDonorLevelConfig(donorAmount);

          return (
            <div
              key={donor?.id || index}
              className="flex items-center space-x-3 p-3 bg-background rounded-lg hover:bg-card transition-colors duration-200"
            >
              {/* Ranking Number */}
              <div className="flex-shrink-0 w-6 text-center">
              ...
              <div className="relative">
                ...
                />
                <div
                  className={`absolute -top-1 -right-1 w-5 h-5 ${levelConfig.bg} rounded-full flex items-center justify-center`}
                >
                  <Icon
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-semibold text-primary">
                    {formatAmount(donorAmount)}
                  </span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${levelConfig.bg} ${levelConfig.color}`}
                  >
                    {levelConfig.level}
                  </span>
                </div>
        ...
        <div className="space-y-2">
          {[
            ...
            >
              <div className="flex items-center space-x-2">
                <div
                  className={`w-2 h-2 rounded-full bg-current ${tier.color}`}
                ></div>
                <span className="text-text-primary">{tier.level}</span>
              </div>
              <span className="text-text-secondary">{tier.amount}</span>
