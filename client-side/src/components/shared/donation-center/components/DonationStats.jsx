...
const DonationStats = ({ stats }) => {
  const statsConfig = [
    {
      label: "Total Raised",
      value: stats?.totalRaised?.value ?? 0,
      icon: "DollarSign",
      color: "primary",
      change: stats?.totalRaised?.change,
      changeType: stats?.totalRaised?.change?.includes("-")
        ? "negative"
        : "positive",
    },
    {
      ...
      icon: "Users",
      color: "success",
      change: stats?.totalDonors?.change,
      changeType: stats?.totalDonors?.change?.includes("-")
        ? "negative"
        : "positive",
    },
    {
      ...
      changeType: stats?.activeCampaigns?.change?.includes("-")
        ? "negative"
        : "positive",
    },
    {
      label: "Avg. Donation",
      value: stats?.avgDonation?.value ?? 0,
      icon: "TrendingUp",
      color: "accent",
      change: stats?.avgDonation?.change,
      changeType: stats?.avgDonation?.change?.includes("-")
        ? "negative"
        : "positive",
    },
  ];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
      {statsConfig?.map((stat, index) => (
        <div
          key={index}
          className="bg-card rounded-lg p-6 border border-border elevation-1 hover:elevation-2 transition-all duration-200"
        >
          <div className="flex items-center justify-between">
            <div className="flex-1">
