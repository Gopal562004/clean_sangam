...
//   // 🔄 Keep local saved state in sync when parent updates
//   useEffect(() => {
//     setSaved(isSaved);
//   }, [isSaved]);

//   const handleSave = () => {
//     const jobId = job?._id || job?.id; // ✅ handles both cases
//     const newSaved = !saved;
//     setSaved(newSaved);
//     onSave?.(jobId, newSaved);
//   };
...
//             size={20}
//             className={
//               saved ? "text-primary fill-current" : "text-muted-foreground"
//             }
//           />
//         </button>
//       </div>
...
import Icon from "../../../AppIcon";
import Button from "../../../ui/Button";
import { getApplicationStatus } from "../../../../lib/mongo/jobServices"; // import your Axios function

const JobCard = ({ job, onSave, onApply, onViewDetails, isSaved = false }) => {
  const [isHovered, setIsHovered] = useState(false);
  const [saved, setSaved] = useState(isSaved);
  const [applied, setApplied] = useState(false); // ✅ track if user already applied
  ...
  useEffect(() => {
    const checkApplied = async () => {
      if (!job?._id) return;
      try {
        const { applied } = await getApplicationStatus(job._id);
        setApplied(applied);
    ...
    >
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            {applied ? "Already Applied" : "Apply Now"} {/* ✅ change text */}
          </Button>
        </div>
      </div>
